...
`server_ipv6`) at the Terraform output. Then on the server:

```bash
echo "DOMAIN=your-domain.example" >> /opt/video-service/.env
```
...
drift apart. Generate an access key pair for the bucket and put them in `.env`
(`APP_STORAGE_*`, see `.env.example`).

**Storage Box (I3, backups).** Order one in the Hetzner Robot console (separate from the Cloud
Console - Storage Box is a Robot product). Then:
...
## 3. Restore test (do this once, for real)
